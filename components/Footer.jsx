...
const Footer = () => {
    ...
    const YoutubeIcon = () => (<svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg"> <path d="M18.167 5.58301C17.9705 4.80454 17.3926 4.19029 16.6628 3.98301C15.3378 3.60801 10.0013 3.60801 10.0013 3.60801C10.0013 3.60801 4.66464 3.60801 3.33964 3.98301C2.60983 4.19029 2.03199 4.80454 1.83546 5.58301C1.5013 6.99486 1.5013 9.94134 1.5013 9.94134C1.5013 9.94134 1.5013 12.8878 1.83546 14.2997C2.03199 15.0781 2.60983 15.6924 3.33964 15.8997C4.66464 16.2747 10.0013 16.2747 10.0013 16.2747C10.0013 16.2747 15.3378 16.2747 16.6628 15.8997C17.3926 15.6924 17.9705 15.0781 18.167 14.2997C18.5013 12.8878 18.5013 9.94134 18.5013 9.94134C18.5013 9.94134 18.5013 6.99486 18.167 5.58301Z" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" /> <path d="M8.33464 12.5003L12.668 9.94199L8.33464 7.38366V12.5003Z" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" /> </svg>)
    ...
    return (
        <footer className="border-t border-[#123f19]/15 bg-white text-[#484848]">
            <div className="mx-6">
                <div className="max-w-7xl mx-auto">
                    <div className="flex flex-col lg:flex-row items-start justify-between gap-8 py-10 border-b border-[#123f19]/15">
                        <div className="flex flex-col sm:flex-row items-start gap-6">
                            <Link href="/">
                                <Image src={logo} alt="Solar Chapter" className="h-20 w-auto rounded-lg" />
                            </Link>
                            <p className="max-w-[410px] text-sm leading-6 text-[#6b6255]">Solar Chapter is a US 501 (c)(3) public charity | Yayasan Solar Chapter adalah yayasan yang telah sah terdaftar pada Departemen Hukum dan HAM Republik Indonesia</p>
                        </div>
                        <div className="flex flex-wrap justify-between w-full lg:w-auto gap-5 text-sm">
                            {linkSections.map((section, index) => (
                                <div key={index} className="lg:min-w-80">
                                    <h3 className="font-semibold uppercase tracking-widest text-xs text-[#173d1f] mb-3">{section.title}</h3>
                                    {section.title === "CONTACT" && (
                                        <div className="flex items-center gap-3">
                                            {socialIcons.map((item, i) => (
                                                <Link href={item.link} key={i} target="_blank" rel="noopener noreferrer" className="flex items-center justify-center w-9 h-9 bg-[#123f19]/8 text-[#123f19] hover:bg-[#123f19]/15 transition rounded-full">
                                                    <item.icon />
                                                </Link>
                                            ))}
                                        </div>
                    ...
                    <div className="flex flex-col gap-2 py-4 text-sm text-[#6b6255] sm:flex-row sm:items-center sm:justify-between">
                        <p>© Solar Chapter {new Date().getFullYear()}</p>
                        <Link href="/privacy" className="hover:text-[#123f19] transition">
                            Privacy Policy
                        </Link>
                    </div>
                </div>
